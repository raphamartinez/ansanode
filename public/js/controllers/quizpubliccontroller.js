...
const salutation = async () => {
    ...
    var token = url.substring(12)

    const quiz = await Connection.noBody(`quiz/${token}`, 'GET')

    if (quiz.status === false) window.location.href = "http://www.americaneumaticos.com.py/"
...
const finish = async (event) => {
    ...
    window.location.href = "http://www.americaneumaticos.com.py/"
}
