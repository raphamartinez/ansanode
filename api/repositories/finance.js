...
class Finance {
    ...
    async check(finance) {
        try {
            const sql = `SELECT * FROM financeinvoice WHERE invoicenr = ? AND datereg > NOW() - interval 12 hour`
    ...
    async checkExpected(finance) {
        try {
            const sql = `SELECT * FROM financeexpected WHERE client = ? AND office = ?`
    ...
    async update(finance, id_login) {
        try {
            const sql = `UPDATE financeinvoice SET contactdate = ?, responsible = ?, contact = ?, comment = ?, payday = ?, status = ?, id_login = ? WHERE id_financeinvoice = ?`
    ...
    async updateExpected(finance) {
        try {
            const sql = `UPDATE financeexpected SET transfUsd = ?, chequeUsd = ?, transfGs = ?, chequeGs = ?, date = ?  WHERE id = ?`
    ...
    async listClient(client, d1, d2) {
        try {
            ...
            FROM receivable re
            LEFT JOIN financeinvoice fi ON re.SerNr = fi.invoicenr `
    ...
    listInvoiceHistory(invoice) {
        try {
            ...
            INNER JOIN user us ON fi.id_login = us.id_login
            WHERE invoicenr = ?
    ...
    graphReceivable(id_login, offices, datestart, dateend) {
        try {
            ...
            if (offices) sql += ` AND fi.id_login IN ( SELECT ou.id_login FROM officeuser ou
    ...
    listOpenInvoices(period, arroffices) {
        ...
        try {
            ...
            FROM office as ofi 
            INNER JOIN receivable re on ofi.code = re.Office
    ...
    async listResumeOffice(arroffices, month) {
        try {
            ...
            FROM office as ofi
            LEFT JOIN financeinvoice as fi on ofi.code = fi.office and LAST_DAY(fi.contactdate) = LAST_DAY(?)
            LEFT JOIN financeexpected as fe on fi.client = fe.client and LAST_DAY(fe.date) = LAST_DAY(?)
